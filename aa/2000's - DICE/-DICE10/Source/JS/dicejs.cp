...
2003.03.15	jht:	Extracted from js.c.
					My modes marked by jht_mod
*/

// 2003.03.20 mark my mods
#define jht_mod 1
...
#include "jsfun.h"
#include "jsgc.h"
#include "jslock.h"		// !!@ 2003.03.21 jht: Note not extern c macro
#include "jsobj.h"
#include "jsparse.h"
#include "jsscope.h"
#include "jsscript.h"
...
#define fprintf 	dicejs_fprintf
#define fputs 		dicejs_fputs
#define fwrite		dicejs_fwrite
#define fputc		dicejs_fputc


#endif

// !!@ 2003.03.21 jht: jht_mod 
...
{
#if 0
// 2004.12.13 js_SearchScope link problem
    uintN i;
    JSScope *scope;
...
static JSBool its_noisy = 1;    /* whether to be noisy when finalizing it */
...
#ifdef jht_mod
// --------------------------------------------------------------------------------
// 2003.03.21 jht: My Additions to original js.c

static int			js_inited = 0;
static JSVersion 	version = JSVERSION_DEFAULT;
static JSRuntime 	*rt = 0;
static JSContext 	*cx = 0;
static JSObject 	*glob = 0;
static JSObject 	*it = 0;
...
// --------------------------------------------------------------------------------
void dicejs_EvalStringPrint( char *buffer, int len )
{
    JSBool ok;
    JSScript *script;
    jsval result;
    ...
    /* Clear any pending exception from previous failed compiles.  */
    JS_ClearPendingException(cx);
    script = JS_CompileScript(cx, glob, buffer, len,
#ifdef JSDEBUGGER
				                  "typein",
#else
				                  NULL,
    ...
    if (script) 
    {
        ok = JS_ExecuteScript(cx, glob, script, &result);
        if (ok && result != JSVAL_VOID) 
        {
            str = JS_ValueToString(cx, result);
            if (str)
...
{
	// !!@ Assert obj is private value of wrapperObj
	if (! cx)
		return;
		
	if (wrapperObj)
	{
		JS_SetPrivate(cx, (JSObject*)wrapperObj, 0);
	}
}

// --------------------------------------------------------------------------------
void dicejs_GetRuntime( void **xrt, void **xcx, void **xglob)
{
	if (xrt) *xrt = rt;
	if (xcx) *xcx = cx;
	if (xglob) *xglob = glob;
